...
<div id="main" class="wrap">
	<div class="lefter">
		...
		<div class="main">
		<h2 style="font-size:20px;" >修改商品</h2>
		<div class="manage">
			<form action="ProductServlet?method=update" method="post" enctype="multipart/form-data">
				<table class="form">
					<tr>
						<td class="field">当前用户名:</td>
						<td><input type="text" class="text" name="uid"  value="${user.getId() }" readonly/></td>
						<td><input type="hidden" name="id" value="${sessionScope.productUpdate.getId() }"/></td>
						<td><input type="hidden" name="valid" value="${sessionScope.productUpdate.getValid() }"/></td>
					</tr>
					<tr>
						<td class="field">商品名称：</td>
						<td><input type="text" class="text" name="productName" value="${sessionScope.productUpdate.getName() }" /></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input type="file" class="text" name="photo" value="${sessionScope.productUpdate.getPicture() }"/></td>
					</tr>
					<tr>
						<td class="field">商品价格：</td>
						<td><input type="text" class="text tiny" name="productPrice" value="${sessionScope.productUpdate.getPrice() }"/> 元(整数)</td>
					</tr>
					<tr>
						<td class="field">描述：</td>
						<td><input type="text" class="text" name="productDesc" value="${sessionScope.productUpdate.getIntroduction() }"/></td>
					</tr>
					<tr>
						<td class="field">库存：</td>
						<td><input type="text" class="text tiny" name="productStock" value="${sessionScope.productUpdate.getStock() }"/></td>
					</tr>
					<tr>
						<td><a href="ProductServlet?method=myProduct&uid=${user.getId() }">返回</a></td>
						<td><label class="ui-blue"><input type="submit" name="submit" value="提交修改" /></label></td>
					</tr>
				</table>
			</form>
		</div>
